...
void app_main(void)
{
    ...
    /* d) Health monitor                                                   */
    /* ================================================================== */
    /* Started after the watchdog so it can be registered for monitoring. */
    ...
    if (ret != ESP_OK)
    {
        ...
        /* Continue — non-fatal, but the health task won't be monitored. */
    }
    else
    {
        /* Register with watchdog only after task creation succeeded. */
    ...
    /* ================================================================== */
    /* e) Scanner data receiver (Stage C1)                                 */
    /* ================================================================== */

    /* C1: no parser task yet, so pass NULL — the receiver will only echo.
     * In C2 we will create a queue and hand it here. */
    ret = scan_receiver_init(NULL);
    if (ret != ESP_OK)
    {
        output_write("{\"type\":\"warning\",\"msg\":\"scan_receiver init failed\"}");
